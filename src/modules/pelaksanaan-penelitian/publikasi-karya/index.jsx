				...
				<thead>
					<tr>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("publikasi_id")}>
								No
								<SortIcon sort={getSortBy("publikasi_id")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("judul_artikel")}>
								Judul
								<SortIcon sort={getSortBy("judul_artikel")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("nama_jurnal")}>
								Nama Jurnal
					...
					{!loading &&
						...
						data.map((row, index) => (
							<tr key={`row-${index}`}>
								...
								<td className="text-sm border-2 border-white bg-gray-50 max-w-[12rem] truncate">{row.judul_artikel}</td>
