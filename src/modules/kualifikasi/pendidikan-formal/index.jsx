				...
				<thead>
					<tr>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("pend_id")}>
								No
								<SortIcon sort={getSortBy("pend_id")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("jenjang_studi")}>
								Jenjang
								<SortIcon sort={getSortBy("jenjang_studi")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("gelar_akademik")}>
								Gelar
								<SortIcon sort={getSortBy("gelar_akademik")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("asal")}>
								Perguruan Tinggi
								<SortIcon sort={getSortBy("asal")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("bidang_studi")}>
								Bidang Studi
								<SortIcon sort={getSortBy("bidang_studi")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("thn_lulus")}>
								Tahun Lulus
					...
					{!loading &&
						...
						data.map((row, index) => (
							<tr key={`row-${index}`}>
								...
								<td className="text-sm border-2 border-white bg-gray-50">{row.program_studi}</td>
