...
export default function PenelitianCreate() {
	...
	useEffect(() => {
		...
		setForm((state) => ({
			...state,
			[`anggota_penelitian_${String(user?.role).toLowerCase()}`]: [{ ...INITIAL_ANGGOTA, user_id: user?.user_id, role: user?.role }],
		}));
	}, [user]);
									...
									<div className="flex items-stretch gap-1">
										{(index > 0 || user?.role != ROLE_ID_MAHASISWA) && (
											<Button.Icon
												...
												variant="danger"
												icon={<Icon icon="solar:trash-bin-2-bold-duotone" width={20} height={20} />}
												onClick={() => removeFromUser("anggota_penelitian_mahasiswa", index, "Mahasiswa")}
											/>
										)}
									</div>
								</td>
